...
void usage(char *prog_name) {
	fprintf(stderr,"Usage: %s [--dim=N] [--ratio=P] [--dev=cpu/gpu] [--with-progress=0/1] [--progress-thread=0/1] [--min-bytes=N] [--max-bytes=N] [--compute-bound=0/1] [--memory-mode=MODE]\n\n", prog_name);
	...
	fprintf(stderr,"--progress-thread (GPU only): 1 for launching one thread per rank dedicated solely to progress, 0 to use the default CPU Polling if --with-progress was enabled \n\n");
...
int run_overlap_benchmark_gpu(int rank, int size, int dim, int compToPureCommRatio, size_t min_bytes, size_t max_bytes,int do_progress, int enable_thread, int compute_bound, memory_mode_t memory_mode) {
	...
	*/
	...
	MPI_Barrier(MPI_COMM_WORLD);
	if(compute_bound==1){
		gpu_inner_iters=calibrate_inner_iter(d_a,stream,grid,block,VECTOR_DIM_COMP,5,&measured_unit_us);
	}
	else{
		mem_cal=calibrate_memory_bound_kernel(d_c,d_a,d_b,stream,grid,block,elems_per_pass,max_elems,400,memory_mode);
		/*
		if (rank == 0) {
	...
	if(do_progress&&enable_thread){
			progress_data.do_progress=1;
			progress_data.is_thread=1;
			progress_data.cuda_device=local_rank%device_count;
			start_progress_thread(&progress_data);
	}
	else if(do_progress&&!enable_thread){
		progress_data.do_progress=1;
		progress_data.is_thread=0;
	}
	else if(!do_progress&&!enable_thread){
		progress_data.do_progress=0;
		progress_data.is_thread=0;
	}
    for (size_t local_N=min_bytes;local_N <= max_bytes; local_N *= 2){
        char *send_buffers[6];
